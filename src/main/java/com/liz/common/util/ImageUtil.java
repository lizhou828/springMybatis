...
public class ImageUtil {
    ...
    public static void resize(String srcImgPath, String outImgPath,
        ...
        BufferedImage src = InputImage(srcImgPath); // 得到图片
        int old_w = src.getWidth(); // 得到源图宽
        int old_h = src.getHeight(); // 得到源图长
		//if (new_w >= old_w) new_w = old_w;
		//if (new_h >= old_h) new_w = old_h;
     ...
     * @author zlq
     */
	public static void cut(File srcfile, File outfile, int x, int y, int width, int height, String fileFormat) throws IOException {
		...
		try {
			 ...
			 */
			Iterator<ImageReader> it = ImageIO.getImageReadersByFormatName(fileFormat);
			...
			// 保存新图片
			ImageIO.write(bi, fileFormat, outfile);
		} finally {
			if (is != null) {
				is.close();
			}
	 ...
	 * 按长宽小的尺寸剪切成正方形
	 * @param file 源图片
	 * @return cut image file 剪切后图片,和源图片在同一目录下
	 * @throws java.io.IOException
	 * @author lmj
	 */
	public static File imageCut(File file) throws IOException {
		String fileName = file.getPath();

		String fileFormat = fileName.substring(fileName.lastIndexOf(".") + 1);


		String outPutName = fileName.substring(0, fileName.indexOf(".")) + "_cut" + "." + fileFormat;
		...
		if(width > height){
			...
			ImageUtil.cut(file, outPutFile, length, 0, height, height, fileFormat);
		}
		if(width < height){
			int length = (height - width)/2;
			ImageUtil.cut(file, outPutFile, 0, length, width, width, fileFormat);
		}
	...
	public final static void pressImage(String pressImg, String targetImg,
		...
		if (x == -1) x = (width - wideth_biao) / 2;
		if (y == -1) y = (height - height_biao) / 2;
